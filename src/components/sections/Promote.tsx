  ...
  return (
    <section className="py-20 px-4" aria-label="Promotion section">
      <div className="max-w-6xl mx-auto">
        <div className="bg-white dark:bg-[#0b101c] rounded-2xl p-12 border border-green-600/20 dark:border-green-500/15 relative overflow-hidden">
          {/* Faint glow accent */}
          <div
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center relative">
            <div>
              <p className="font-mono text-xs text-green-700/80 dark:text-green-400/80 mb-3">
                $ systemctl status monineath.service
                <span className="text-green-600 dark:text-green-400 ml-2">● active (running)</span>
              </p>
            ...
            <div>
              ...
              <dl className="grid grid-cols-4 gap-3">
                {stats.map(s => (
                  <div key={s.label} className="rounded-lg border border-slate-900/[0.08] dark:border-white/[0.07] bg-slate-900/[0.02] dark:bg-white/[0.02] px-3 py-2.5 text-center">
                    <dt className="font-mono text-[10px] uppercase tracking-wider text-gray-500 order-2">{s.label}</dt>
                    <dd className="font-mono text-sm md:text-base font-semibold text-green-700 dark:text-green-400">{s.value}</dd>
                  </div>
                ))}
              </dl>
            </div>
