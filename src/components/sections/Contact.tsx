...
export function Contact() {
  ...
  const inputCls = 'w-full bg-[#0a0e16] border border-white/10 rounded-lg px-4 py-3 text-white font-mono text-sm placeholder-gray-600 focus:outline-none focus:border-green-500 focus:shadow-[0_0_0_3px_rgba(34,197,94,0.08)] transition-all'
  const labelCls = 'block font-mono text-gray-400 text-xs mb-1.5'
    ...
    <section id="contact" className="py-20 px-4" aria-label="Contact section">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-12 items-start">
          <div>
            <p className="font-mono text-xs text-green-400/80 mb-3">$ ssh monineath@anywhere</p>
            ...
            <ul className="space-y-6" role="list">
              {contactItems.map(item => (
                <li key={item.label} className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-lg bg-green-600/15 border border-green-500/30 flex items-center justify-center text-green-400 flex-shrink-0">
                    {item.icon}
                  </div>
                  <div>
          ...
          <div className="rounded-xl border border-white/10 bg-[#0d1320] overflow-hidden">
            {/* Request header bar */}
            <div className="flex items-center gap-2 px-5 py-3 bg-white/[0.03] border-b border-white/[0.06] font-mono text-xs">
              <span className="px-2 py-0.5 rounded border font-semibold bg-green-500/15 text-green-400 border-green-500/30">POST</span>
              <span className="text-gray-400">/api/v1/contact</span>
              <span className="ml-auto text-gray-600">Content-Type: application/json</span>
            </div>
            ...
            <form onSubmit={e => e.preventDefault()} className="space-y-4 p-6" noValidate aria-label="Contact form">
              ...
              <p className="font-mono text-[11px] text-gray-600 text-center">expected response: 201 Created</p>
            </form>
          </div>
        </div>
      </div>
